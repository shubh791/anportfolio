      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-6 md:gap-8">
        {projects.map((p, i) => (
          <motion.div
            ...
            className="group relative bg-[#0a0a0a] border border-white/10 rounded-sm p-5 sm:p-6 md:p-8 flex flex-col hover:border-cyan-500/50 transition-all duration-500"
          >
            {/* CATEGORY & DATE */}
            <div className="flex justify-between items-start mb-4 sm:mb-6">
              <div className="flex items-center gap-2 px-2 py-1 bg-white/5 border border-white/5">
                ...
                <span className="text-[8px] sm:text-[9px] font-mono font-bold uppercase tracking-widest text-gray-500">
                  {p.category}
                </span>
              </div>
              <span className="text-[8px] sm:text-[9px] font-mono text-gray-700">
                {p.date}
              </span>
            </div>

            {/* TITLE */}
            <h3 className="text-lg sm:text-xl font-black text-white italic uppercase tracking-tighter mb-2 sm:mb-3 group-hover:text-cyan-400 transition-colors">
              {p.title}
            </h3>

            {/* DESC */}
            <p className="text-[10px] sm:text-[11px] text-gray-500 leading-relaxed mb-3 sm:mb-4 line-clamp-2">
              {p.desc}
            </p>

            {/* IMPACT */}
            <div className="mb-4 sm:mb-6 p-2 sm:p-3 bg-cyan-500/5 border-l-2 border-cyan-500">
              <p className="text-[9px] sm:text-[10px] font-mono text-cyan-500 font-bold uppercase tracking-tighter">
                {p.impact}
              </p>
            </div>

            {/* TECH */}
            <div className="flex flex-wrap gap-2 mb-6 sm:mb-8 mt-auto">
              {p.tech.map((t, idx) => (
                <span
                  key={idx}
                  className="text-[8px] sm:text-[9px] font-mono text-gray-500 border border-white/5 px-2 py-1"
                >
                  {t}
                </span>
              ))}
            ...
            <div className="border-t border-white/5 pt-4 sm:pt-6">
              <a
                ...
                className="inline-flex items-center gap-2 text-[9px] sm:text-[10px] font-black uppercase tracking-widest text-white hover:text-cyan-400 transition-colors"
              >
